...
/*	@Override
	...
	public Client update(Client client){
		...
		try {
			String sql = "UPDATE client SET nom_client=?,prenom_client=?"
					+ ",email_client=?,adresse_client=?, code_postal=?,ville_client=?,telephone_client=?,profession_client=?  WHERE id_client=?";
	...
	public List<Client> findClientsDuConseiller(Employer conseiller) {
		...
		try {
			String sql = "SELECT * FROM client WHERE id_conseiller=?";
			prepst = connect.prepareStatement(sql);
			prepst.setLong(1, conseiller.getId_employer());
			ResultSet rs = prepst.executeQuery();
			while (rs.next()) {
				Client client = new Client();
				client.setId_client(rs.getLong("id_client"));
	...
	try {
		String sql = "SELECT * FROM client  WHERE id_client IN ( SELECT MAX(id_client) FROM client) ORDER BY id_client";
		prepst = connect.prepareStatement(sql);
			// prepst.setLong(1, id);
			ResultSet rs = prepst.executeQuery();
			if (rs.first()) {

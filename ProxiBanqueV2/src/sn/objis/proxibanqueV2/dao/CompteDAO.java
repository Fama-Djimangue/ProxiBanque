	...
	public void create(Compte compte, Client client) {
		...
		try {
			String sql = "INSERT INTO compte (numero_compte, solde_compte,"
					...
					+ "?,'"+ client.getId_client() + "')";
			long idcompte = this.dernierId();

			compte.setNumero_compte(MethodUtils.genererNumero(idcompte, compte.getType_compte()));
			System.out.println("NUMCPT: "+compte.getNumero_compte());
...
*/
	public void create(Compte compte, Client client) {
		...
		try {
			...
			long idcompte = this.dernierId()+1;
	...
	public Compte update(Compte compte) throws DataAccessException {
		...
		try {
			...
			compte.setNumero_compte(MethodUtils.genererNumAgence(compte.getId_compte(), compte.getType_compte()));
			prepst = connect.prepareStatement(sql);
	...
	public List<Compte> findByIdClient(long id) {
		...
		try {
			...
			while (rs.next()) {
				...
				compte.setDate_fermeture(rs.getDate("date_fermeture"));				
				compte.setDecouvert(rs.getDouble("decouvert"));
				compte.setId_client(rs.getLong("id_client"));
	...
	public long dernierId() {
		...
		String reqId = "SELECT MAX(id_compte)as dernierCompte FROM compte";
		...
		try {
			...
			if (rs.first()) {
				IdCompte = rs.getLong("dernierCompte");
			}

...
public class PanelTransactionRetrait extends JPanel {
	 ...
	 * Create the panel.
	 */
	public PanelTransactionRetrait() {
		setLayout(new BorderLayout(0, 0));
		
		JPanel panelHaut = new JPanel();
		add(panelHaut, BorderLayout.NORTH);
		
		JLabel lblDepot = new JLabel("Formulaire Retrait");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!areEmpty){
					...
					if(compte!=null){
						...
						newRetrait= new Transaction(numeroSaisi, montant, "RETRAIT");
						
							tcr = new ThreadCompteRetrait("ThreadRetrait", compte, montant);
							tcr.start();
							transactionService.create(newRetrait);							
							transactionModele.addTransactionInModele(newRetrait);
							long iTrans = transactionService.dernierIdTrans(newRetrait);
							newRetrait.setId_transact(iTrans);
							transactionCompteServive.createTransaction(newRetrait, compte);

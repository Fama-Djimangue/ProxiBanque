...
Pour lancer phpMyAdmin vous pouvez aussi saisir directe sur votre navigateur l'url suivant: http://localhost/phpmyadmin/
	Dans Mysql, importer la base données "proxibanquev3" suivant les inctructions suivantes:
	-sur l'onglet "Base de Données" dans le champ "Créer une base de données" saisir "proxibanquev3" 
	-sur l'onglet "importer", cliquer "choisir un fichier" puis placer vous dans le dossier 'Livraison'
	 et choisir le fichier 'proxibanquev3.sql' puis cliquer sur le bouton "Exécuter" en bas de page.	 
...
Notez la dernière ligne (INFO : Server Startup in XXXX ms ) prouvant que le serveur est bien démarré et précisant le temps de démarrage en millisecondes .
 ...
 Copiez le fichier ProxiBanquev3.war dans le répertoire webapps de déploiement de Tomcat. Cela revient à déployer l’application.
...
Avec votre navigateur , allez sur l’adresse de l’application : http://localhost:8080/ProxiBanquev3

Pour tester l'application:

- Se connecter en tant que Gérent : seul un gérent a la possibilité d'ajouter un Conseiller, il a accés à la liste des conseillers et de ses propres clients

login : admin 
Mot de passe : admin

- Se connecter en tant que Conseiller : un Conseiller a sa propre liste de client, ouvrir un compte pour un client et faire des opérations sur les comptes.
